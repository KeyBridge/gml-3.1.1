...
/**
 * Direct representation of a temporal position. Indeterminate time values are
 * also allowed, as described in ISO 19108. The indeterminatePosition attribute
 * can be used alone or it can qualify a specific value for temporal position
 * (e.g. before 2002-12, after 1019624400). For time values that identify
 * position within a calendar, the calendarEraName attribute provides the name
 * of the calendar era to which the date is referenced (e.g. the Meiji era of
...
public class TimePositionType {
   ...
   * collapsed by defining a union of XML Schema simple types for indicating
   * temporal position relative to a specific reference system.
   * <p/>
   * Dates and dateTime may be indicated with varying degrees of precision.
   * dateTime by itself does not allow right-truncation, except for fractions of
   * seconds. When used with non-Gregorian calendars based on years, months,
   * days, the same lexical representation should still be used, with leading
   * zeros added if the year value would otherwise have fewer than four digits.
   * <p/>
   * An ordinal position may be referenced via URI identifying the definition of
   * an ordinal era.
   * <p/>
   * A time coordinate value is indicated as a decimal (e.g. UNIX time, GPS
   * calendar).Gets the value of the value property.
   * <p/>
   ...
   * <p/>
   * @param value allowed object is {@link TimeIndeterminateValueType }
   *
   */
